...
@section('content')
        ...
        <div class="card-body d-flex flex-wrap justify-content-between align-items-center gap-2">
            ...
            <div class="d-flex flex-wrap gap-2">
                ...
                @if($editable)
                    ...
                    <form method="post" action="{{ route('admin.field-selection-plans.publish', $plan) }}" onsubmit="return confirm('آیا از انتشار این لیست انتخاب رشته مطمئن هستید؟')">@csrf<button class="btn btn-success" @disabled($itemsCount === 0) title="{{ $itemsCount === 0 ? 'برای انتشار حداقل یک رشته ثبت کنید.' : '' }}"><i class="ri-send-plane-line"></i> انتشار لیست</button></form>
    ...
    <div class="card mb-3">
        ...
        <div class="card-body">
            <div class="catalog-filter-grid">
                <div>
                    <label class="form-label" for="catalog-field-query">نام رشته یا کدرشته</label>
            ...
            <div class="catalog-help">رشته را تایپ کن؛ استان، شهر و نوع دانشگاه/دوره فقط از گزینه‌های موجود دیتابیس انتخاب می‌شوند. از نتایج می‌توانی مستقیم به لیست اضافه کنی.</div>
    ...
    <div class="card">
        <div class="card-header d-flex flex-wrap justify-content-between align-items-center gap-2">
            ...
            <input class="form-control selection-search" id="selection-filter" placeholder="جستجو در کد رشته، نام رشته یا شهر..." aria-label="جستجو در انتخاب رشته">
        </div>
        <div class="card-body">
            @if($itemsCount === 0)
            ...
            <div class="selection-columns" id="selection-columns">
                <section class="selection-column"><div class="selection-column-head">ستون راست: ردیف ۱ تا ۷۵</div><div class="table-responsive"><table class="selection-table"><thead><tr>@if($editable)<th></th>@endif<th>ردیف</th><th>کد رشته</th><th>نام رشته</th><th>شهر</th>@if($editable)<th>عملیات</th>@endif</tr></thead><tbody class="selection-list" data-list="right">@foreach($plan->items->take(75) as $item) @include('admin.field-selection._item', ['item' => $item, 'editable' => $editable]) @endforeach @if($editable && $itemsCount < 75) @include('admin.field-selection._add-row') @endif</tbody></table></div></section>
                <section class="selection-column"><div class="selection-column-head">ستون چپ: ردیف ۷۶ تا ۱۵۰</div><div class="table-responsive"><table class="selection-table"><thead><tr>@if($editable)<th></th>@endif<th>ردیف</th><th>کد رشته</th><th>نام رشته</th><th>شهر</th>@if($editable)<th>عملیات</th>@endif</tr></thead><tbody class="selection-list" data-list="left">@foreach($plan->items->slice(75) as $item) @include('admin.field-selection._item', ['item' => $item, 'editable' => $editable]) @endforeach @if($editable && $itemsCount >= 75 && $itemsCount < 150) @include('admin.field-selection._add-row') @endif</tbody></table></div></section>
            </div>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function setCount(count = itemRows().length) { document.getElementById('item-counter').textContent = formatter.format(count); document.querySelectorAll('[data-catalog-add]').forEach(button => button.disabled = count >= 150); document.querySelectorAll('[data-add-row] input, [data-add-row] button').forEach(control => control.disabled = count >= 150); }
    ...
    function appendItem(payload) { const template = document.createElement('template'); template.innerHTML = payload.html.trim(); const row = template.content.firstElementChild; const target = itemRows().length < 75 ? lists[0] : lists[1]; target.insertBefore(row, target.querySelector('[data-add-row]')); document.querySelector('.empty-selection')?.remove(); bindRow(row); rebalance(); clearDirty(); showToast(payload.message); }
    ...
    function renderCatalog(fields) { const results = document.getElementById('catalog-results'); results.replaceChildren(); if (!fields.length) { setCatalogState('موردی با این فیلترها پیدا نشد.'); return; } const summary = document.createElement('div'); summary.className = 'catalog-state'; summary.textContent = fields.length >= 30 ? '۳۰ مورد اول نمایش داده شده؛ برای نتیجه دقیق‌تر نام رشته یا شهر را محدودتر کن.' : formatter.format(fields.length) + ' رشتهمحل پیدا شد.'; results.append(summary); fields.forEach(field => { const row = document.createElement('div'); row.className = 'catalog-result'; row.dataset.catalogResult = '1'; row.dataset.code = field.field_code; const code = document.createElement('span'); code.className = 'catalog-code ltr'; code.textContent = field.field_code; const main = document.createElement('div'); const title = document.createElement('strong'); title.className = 'catalog-title'; title.textContent = field.field_name; const meta = document.createElement('div'); meta.className = 'catalog-meta'; [field.exam_group, field.course_type, field.capacity ? formatter.format(field.capacity) + ' نفر ظرفیت' : null].filter(Boolean).forEach(text => { const chip = document.createElement('span'); chip.className = 'catalog-chip'; chip.textContent = text; meta.append(chip); }); main.append(title, meta); const place = document.createElement('div'); place.className = 'catalog-result-place'; const inst = document.createElement('div'); inst.textContent = field.institution || '-'; const city = document.createElement('div'); city.className = 'catalog-place'; city.textContent = [field.province, field.city].filter(Boolean).join(' / '); place.append(inst, city); const action = document.createElement('div'); action.className = 'catalog-result-action'; const add = document.createElement('button'); add.type = 'button'; add.className = 'btn btn-sm btn-primary'; add.dataset.catalogAdd = field.id; add.textContent = 'افزودن به لیست'; add.disabled = itemRows().length >= 150; action.append(add); row.append(code, main, place, action); results.append(row); }); }
    ...
    document.getElementById('selection-filter')?.addEventListener('input', function () { const needle = this.value.trim().toLowerCase(); itemRows().forEach(row => { const values = [...row.querySelectorAll('input')].map(input => input.value).join(' '); row.hidden = needle && !(row.textContent + ' ' + values).toLowerCase().includes(needle); }); });
    ...
    document.getElementById('bulk-update-form')?.addEventListener('submit', async function (event) { event.preventDefault(); try { const payload = await jsonRequest(this.action, {method:'POST', body:new FormData(this)}); clearDirty(); showToast(payload.message); } catch (error) { showToast(error.message, 'danger'); } });
    document.getElementById('reorder-form')?.addEventListener('submit', async function (event) { event.preventDefault(); this.querySelectorAll('input[name="ordered_item_ids[]"]').forEach(input => input.remove()); itemRows().forEach(row => { const input = document.createElement('input'); input.type = 'hidden'; input.name = 'ordered_item_ids[]'; input.value = row.dataset.itemId; this.append(input); }); try { const payload = await jsonRequest(this.action, {method:'POST', body:new FormData(this)}); clearDirty(); showToast(payload.message); } catch (error) { showToast(error.message, 'danger'); } });
