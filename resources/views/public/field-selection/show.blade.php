...
@push('styles')
<style>.field-columns{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:1rem}.field-table{width:100%;min-width:900px;font-size:.8rem}.field-table th,.field-table td{padding:.5rem;border-bottom:1px solid var(--border);vertical-align:top}.field-table th{color:var(--ink-500)}@media(max-width:767.98px){.field-columns{grid-template-columns:1fr}}</style>
@endpush
...
@section('content')
    <div class="card mt-3"><div class="card-body"><div class="section-title"><i class="ri-list-ordered"></i> لیست انتخاب رشته</div>
        ...
        @else
            ...
            <div class="field-columns">@foreach([$plan->items->take(75), $plan->items->slice(75)] as $items)<div class="table-responsive"><table class="field-table"><thead><tr><th>ردیف</th><th>کد رشته</th><th>نام رشته</th><th>توضیحات رشته</th><th>دانشگاه</th><th>شهر</th><th>نوع دانشگاه</th></tr></thead><tbody>@foreach($items as $item)<tr><td>{{ \App\Support\PersianDate::number($item->priority_order) }}</td><td>{{ $item->field_code }}</td><td>{{ $item->field_name }}</td><td>{{ $item->field_description ?: $item->university_description ?: '-' }}</td><td>{{ $item->university_name ?: '-' }}</td><td>{{ $item->city }}</td><td>{{ $item->university_type ?: '-' }}</td></tr>@endforeach</tbody></table></div>@endforeach</div>
        @endif
    </div></div>
@endsection
